...
class ModelOutputProcessor:
    ...
    def extract_sentences_from_batch(self, batch: BatchData):
        ...
        for idx in range(
            ...
            sentence_mask = (
                batch.input_ids[idx] != 0
            )  # Create a mask where the input_ids are not padding
            ...
            if sentence_mask.any():
                ...
                sentence_loss = active_losses[
                    loss_start_idx : loss_start_idx + actual_token_count
                ]
...
class ModelOutputWorkflowManager:
    ...
    def process_model_outputs(self, split):
        ...
        for split in self.splits:
            if self.get_batch_size(
                split
            ):  # Ensure the split has a designated batch size
    ...
    def get_split_data(self, split):
        ...
        dataloader = self.data_manager.get_dataloader(
            split, batch_size, self.config.num_workers
        )
        if dataloader:
            sentences = ModelOutputProcessor(
                dataloader, self.model, self.device
            ).process_outputs()
            return sentences
        else:
            logging.warning(
                "No data available for %s split, returning empty list.", split
            )
            return []
...
class PretrainedModelOutputProcessor:
    ...
    def process_outputs(self):
        ...
        with torch.no_grad():
            for data in tqdm(self.data_loader):
                model_data = {
                    k: v.to(self.device) for k, v in data.items() if k in valid_keys
                }
...
class PretrainedModelOutputWorkflowManager:
    ...
    def get_split_data(self, split):
        ...
        if dataloader:
            outputs = PretrainedModelOutputProcessor(
                dataloader, self.model, self.device
            ).process_outputs()
            return outputs
        else:
