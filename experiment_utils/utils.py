...
class GenerateData:
    ...
    def generate_anercorp(self, split_data: bool = False, validation_size: float = 0.2) -> Dict[str, Any]:
        """
        ...
        ner_map = {'O': 0, 'B-PERS': 1, 'I-PERS': 2, 'B-ORG': 3, 'I-ORG': 4, 'B-LOC': 5, 'I-LOC': 6, 'B-MISC': 7, 'I-MISC': 8}
        ...
        if split_data:
            train_dt, val_dt = train_test_split(tr_dt, test_size=validation_size, random_state=1)
        else:
            train_dt, val_dt = tr_dt, []

        # Prepare the output datasets
        train = [(sentence_id, sentence[0], sentence[1]) for sentence_id, sentence in enumerate(train_dt)]
        validation = [(sentence_id, sentence[0], sentence[1]) for sentence_id, sentence in enumerate(val_dt)]
        test = [(sentence_id, sentence[0], sentence[1]) for sentence_id, sentence in enumerate(test_dt)]
    ...
    def generate_split_data(self, dataset: Dict[str, Any], split: str, ner_inv_map: Dict[int, str]) -> List[Tuple[int, List[str], List[str]]]:
        ...
        try:
            for i in tqdm(range(len(data_split)), desc=f"Processing {split}"):
                entry = data_split[i]
                entry_id = int(entry.get('id', -1))  # Default to -1 if 'id' is not found
