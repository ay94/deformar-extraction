...
class OutputGenerationPipeline:
    ...
    def load_pretrained_model(self, pretrained_model_path):
        ...
        """
        try:
            logging.info(f"Loading pretrained model from: {self.pretrained_model_path}")
            pretrained_model = AutoModel.from_pretrained(
                self.pretrained_model_path, output_attentions=True, output_hidden_states=True
            )
    ...
    def run(self, split: str) -> Dict[str, object]:
        ...
        try:
            ...
            logging.info(f"Generating model outputs for split: {split}")
            pertrained_model_outputs_manager = ModelOutputWorkflowManager(
